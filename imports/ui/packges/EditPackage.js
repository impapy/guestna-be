...
export class EditPackage extends React.Component {
    ...
    componentDidMount() {
        ...
        fetch(`/ThePackgeAdmin?userId=${Meteor.userId()}&id=${this.props.match.params.id}`)
            .then((res) => res.json())
            .then((json) => {
                this.setState({
                    ...json,
    ...
    addService(e) {
        ...
        this.setState({
            ...
            () => {
                ...
                delete opj.events;
                delete opj.providers;

              //  opj.events = opj.events.map((item , index) => { return item._id })
                

                Meteor.call("editPack", opj, (err, res) => {
                    if (err) {
                        alert(err)
                    } else {
                        alert("done")
                            ...
                            <div className="row" >
                                ...
                                <div className="col-md-2" style={{ textAlign: "right" }}>
                                    ...
                                    <div id="list1" className="dropdown-check-list" tabIndex="100">
                                        <span className="anchor">الانشطة</span>
                                        <ul className="items">
