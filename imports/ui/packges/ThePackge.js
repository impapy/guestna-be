...
export class ThePackge extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            ThePackge: [],
    ...
    componentDidMount() {
        fetch(`/ThePackgeAdmin?id=${this.props.match.params.id}&userId=${Meteor.userId()}`)
            ...
            .then(() => {
                var events = []
                this.state.ThePackge.events.forEach(element => {
                    element = { ...element, dateArray: this.getDates(element.fromDay, element.toDay) }
                    events.push(element)
                ...
                this.setState({
                    ...
                    events: events,
                    dates: this.getDates(this.state.ThePackge.fromDay, this.state.ThePackge.toDay)
                })
            })
            .then(() => {
                var tempArr = []
                this.state.dates.forEach((date) => {
                    tempArr.push({ day: date, events: [] })
                })
                this.setState({
                    DataisLoaded: true,
        ...
        return (
            ...
            <div style={{ textAlign: "center", marginTop: "120px" }} >
                {(this.state.DataisLoaded) ?
                    <form className="userForm" onSubmit={this.setDefaultView.bind(this)}>
